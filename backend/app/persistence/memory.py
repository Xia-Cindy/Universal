...
from backend.app.persistence.codec import dumps, memory_from_payload
...
class SQLiteMemoryRepository:
    ...
    def get(self, memory_id: str, user_id: str) -> MemoryEntry:
        ...
        entry = memory_from_payload(json.loads(row["payload"]))
    ...
    def list_for_user(self, user_id: str) -> list[MemoryEntry]:
        ...
        return [memory_from_payload(json.loads(row["payload"])) for row in rows]
