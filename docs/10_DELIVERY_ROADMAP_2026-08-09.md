...
### 2026-08-13 · RAGFlow 上传前运行时验收

- 新增只读 `POST /api/knowledge/provider/runtime-verification`：以固定小型查询执行已有已完成资料的 embedding/retrieval 探针，返回 `verified` 或 `failed`、检测时间和脱敏原因。
- 探针不接收用户文件、不创建数据集、不上传、不持久化探针记录，也不进入解析队列；没有可用已完成资料时明确失败而不创建测试资料。
- 5180 Knowledge 书架在创建 RAGFlow-backed 资料前调用该接口；失败时显示安全原因并阻止上传，避免新文件长期卡在处理中。
- 真实运行验收：`2026-08-13T18:47:28+08:00`，受控临时 API 实例返回 `200 / verified`；实例仅用于该只读请求后立即停止。
...
- 上传前运行时探针只有在返回 `verified` 时才允许新的 RAGFlow-backed 资料进入队列；`health` 可达本身不再被视为上传条件。
