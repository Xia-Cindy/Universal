...
- Study Goal 资料默认不进入 Work Knowledge。用户只能从关联 Goal 的资料阅读器向指定、未归档的 Work Tech Stack 授权；Work 仅获得原 document ID 的只读引用。资料可关联多个 Goal，授权记录保留明确的源 Goal；仅当该源 Goal 链接被移除、授权被撤销、Tech Stack 归档或资料删除时收回引用，绝不复制 chunk、RAGFlow 数据或注释。
...
1. **外部书架依赖（O4 受阻）。** 当前参考书架在运行时读取外部部署 HTML；网络不可用、参考站点变更或跨域策略变化会影响视觉层。2026-08-13 已核验该部署页与 `thebuggeddev/books` 的实际 `index.html` 完全一致，但上游没有声明或检测到许可证，故不能复制或本地化其实现/资源；仅在作者明确授权或补充兼容许可证后再处理。
...
7. **跨 Planet 引用仍需继续收敛。** Study-to-Work 已具备显式资料授权、撤销和多 Goal 资料关联；更细粒度审计、共享阅读进度与生产级访问控制仍未完成。
...
5. 多 Goal 资料关联已完成；后续只在用户验证边界与参数后演进共享阅读进度或更细粒度审计。Study Session 的事务性完成仍需按已批准的设计实施。
